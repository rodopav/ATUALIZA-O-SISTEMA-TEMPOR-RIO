...
export function KpiCard({
  ...
  return (
    <div
      ...
      className={cn(
        'card relative overflow-hidden p-4 pt-5 transition-all duration-200',
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3">
        <p className="text-[10px] font-bold uppercase tracking-[1.5px] text-zinc-400">{label}</p>
        <div className={cn('flex h-9 w-9 shrink-0 items-center justify-center rounded-lg', TINTA[tom])}>
          {icone}
        </div>
      </div>
      <div className="mt-2 min-h-[36px]">
        {loading ? (
          <div className="h-8 w-28 animate-pulse rounded bg-zinc-800/60" />
        ) : (
          <p className="num-mono text-[26px] font-extrabold leading-none text-zinc-50">{valor}</p>
        )}
      </div>
